...
/*
Finds the degrees of separation
* between Marvel comic book characters
* */

object DegreesOfSeparation {

  val startCharacterID = 5306 // Spider Man
  val targetCharacterID = 2658 //"IRON FIST/DANIEL RAN"

  // We make our accumulator a "global" Option so we can reference it in a mapper later.
  var hitCounter:Option[LongAccumulator] = None

  // Custom Data types
  // BFSData contains an array of heroID connections, the distance and a color
  type BFSData = (Array[Int], Int, String)
  // BFSNode contains a heroID and BFSData associated with it
  type BFSNode = (Int, BFSData)


  /*Convert a line into a BFSNode*/
  ...
  def createStartingRdd(sc:SparkContext): RDD[BFSNode] = {
    val inputFile = sc.textFile("./marvel_data/marvel-graph.txt")
  ...
  def main(args: Array[String]): Unit = {
    ...
    // accumulator used to signal when the target is found during the BFS traversal
    ...
    for (iteration <- 1 to 10) {
      ...
      // mapped.count() action here forces the RDD to be evaluated, and
      // that's the only reason the accumulator is updated.
